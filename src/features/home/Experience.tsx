  ...
  return (
    ...
    >
      <div className="mb-16 md:mb-24 text-center md:text-left">
        <h2 className="font-display text-4xl md:text-[80px] font-bold text-white mb-4 leading-none tracking-[-0.04em]"><DecodingText text="Experience." /></h2>
      ...
      <div className="relative mt-12 md:mt-0">
        ...
        <div className="space-y-16 md:space-y-24">
          {experiences.map((exp, idx) => {
            const isLeftMeta = idx % 2 === 0;
            const meta = (
              <div className={`w-full md:w-1/2 pl-12 md:pl-16 ${isLeftMeta ? 'md:pl-0 md:pr-16 text-left md:text-right mb-6 md:mb-0' : 'md:pr-0 text-left order-1 md:order-2'}`}>
                <span className="font-mono text-xs uppercase tracking-widest text-primary font-bold">{exp.period}</span>
                <h3 className="font-display text-2xl md:text-[40px] font-bold text-white mt-2 leading-tight">{exp.role}</h3>
                <p className="text-white/70 mt-1">{exp.company}</p>
              </div>
            );
            const card = (
              <div className={`w-full md:w-1/2 pl-12 md:pl-16 ${isLeftMeta ? '' : 'md:pl-0 md:pr-16 order-2 md:order-1 mt-6 md:mt-0'}`}>
                <motion.div
                  whileHover={{ x: isLeftMeta ? 10 : -10 }}
                ...
                >
                  <p className="text-white/90 mb-4 font-medium leading-relaxed">{exp.summary}</p>
            ...
            return (
              <div key={`${exp.company}-${exp.period}`} className="relative flex flex-col md:flex-row items-center w-full group">
